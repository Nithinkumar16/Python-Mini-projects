...
# Define an enhanced CNN model for classification
def enhanced_cnn_model(input_size=(128, 128, 1), num_classes=2):
    inputs = layers.Input(input_size)
    x = layers.Conv2D(64, (3, 3), activation='relu')(inputs)  # Increased filter size
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Conv2D(128, (3, 3), activation='relu')(x)  # Increased filter size
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Conv2D(256, (3, 3), activation='relu')(x)  # New additional layer
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Conv2D(512, (3, 3), activation='relu')(x)  # Another new layer
    x = layers.MaxPooling2D((2, 2))(x)

    # Flatten and add Dense layers with increased units
    x = layers.Flatten()(x)
    x = layers.Dense(512, activation='relu')(x)  # Increased Dense layer units
...
def main(image_folder):
    ...
    # Train the model
    history = model.fit(X, y, validation_split=0.2, epochs=10, batch_size=8)  # Increased epochs and batch size
