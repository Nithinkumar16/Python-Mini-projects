...
from tensorflow.keras.layers import LeakyReLU  # Import Leaky ReLU
...
# Define CNN model for classification with added Dropout and L2 Regularization
def simple_cnn_model(input_size=(128, 128, 1), num_classes=2):
    inputs = layers.Input(input_size)

    # First Convolutional Block
    x = layers.Conv2D(32, (3, 3), kernel_regularizer=regularizers.l2(0.001))(inputs)  # L2 Regularization
    x = LeakyReLU(alpha=0.1)(x)  # Leaky ReLU Activation
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Dropout(0.25)(x)  # Dropout Layer

    # Second Convolutional Block
    x = layers.Conv2D(64, (3, 3), kernel_regularizer=regularizers.l2(0.001))(x)  # L2 Regularization
    x = LeakyReLU(alpha=0.1)(x)  # Leaky ReLU Activation
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Dropout(0.25)(x)  # Dropout Layer
    ...
    x = layers.Flatten()(x)
    x = layers.Dense(64, kernel_regularizer=regularizers.l2(0.001))(x)  # Reduced neurons
    x = LeakyReLU(alpha=0.1)(x)  # Leaky ReLU Activation
...
def main(image_folder):
    ...
    history = model.fit(
        ...
        epochs=10,  # Increased epochs to allow early stopping to take effect
